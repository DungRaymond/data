...
async function extract() {
  fs.writeFileSync('./json/modeList56.json', "[\n");
  for await(let each of sample) {
    let pivot = each;
    let data = JSON.parse(fs.readFileSync("./json/collection56/type" + pivot + ".json"));
    let obj = {
      modePivot: pivot,
      modeList: [
        {
    ...
    if(each == 55){
      fs.appendFileSync('./json/modeList56.json', "\t" + JSON.stringify(obj) + "\n")
    } else {
      fs.appendFileSync('./json/modeList56.json', "\t" + JSON.stringify(obj) + ",\n")
    }
    
  }
  fs.appendFileSync('./json/modeList56.json', "\n]");
